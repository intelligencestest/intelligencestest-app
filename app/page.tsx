...
export default async function Home() {
  ...
  return (
    <main className="min-h-screen bg-[#f8fafc] text-slate-100">
      ...
      <section className="relative overflow-hidden border-b border-[#f3f4f6]">
        ...
        <div className="relative mx-auto grid max-w-7xl items-center gap-14 px-5 py-20 sm:px-6 lg:grid-cols-[1.05fr_0.95fr] lg:px-8 lg:py-28">
          <div className="animate-fade-up">
            ...
            <dl className="mt-12 flex max-w-xl divide-x divide-[#f3f4f6] border-t border-[#f3f4f6] pt-6">
              {copy.home.stats.map((stat, i) => (
                <div key={stat.label} className={i === 0 ? "pr-8" : "px-8"}>
                  <dt className="text-2xl font-semibold tracking-tight text-[var(--it-text)]">{stat.value}</dt>
          ...
          <div className="relative hidden lg:block" aria-hidden="false">
            <div className="absolute -inset-10 rounded-[40px] bg-[radial-gradient(ellipse_at_center,rgba(79,70,229,0.06),transparent_65%)]" aria-hidden="true" />
      ...
      <section id="features" className="scroll-mt-20 border-b border-[#f3f4f6] bg-white py-20 sm:py-28">
        <div className="mx-auto max-w-7xl px-5 sm:px-6 lg:px-8">
          <div className="max-w-2xl">
            ...
            <h2 className="mt-3 text-3xl font-semibold tracking-[-0.015em] text-[var(--it-text)] sm:text-4xl">{copy.home.featuresTitle}</h2>
          </div>
          <div className="mt-12 grid gap-5 md:grid-cols-2 lg:grid-cols-4">
            {copy.home.features.map((feature, i) => {
              ...
              return (
                <article
                  key={feature.title}
                  className="group relative overflow-hidden rounded-2xl border border-[#f3f4f6] bg-[#ffffff] p-6 transition-colors duration-200 hover:border-[#4f46e5]/40"
                >
                  <div className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-[#4f46e5]/50 to-transparent opacity-0 transition-opacity duration-300 group-hover:opacity-100" aria-hidden="true" />
                  <div className="mb-6 flex h-10 w-10 items-center justify-center rounded-xl border border-[#4f46e5]/30 bg-[#4f46e5]/10 text-[#4338ca] ring-1 ring-inset ring-white/5">
                    <Icon className="h-[18px] w-[18px]" strokeWidth={1.8} aria-hidden="true" />
                  </div>
                  <h3 className="text-[17px] font-semibold text-[var(--it-text)]">{feature.title}</h3>
      ...
      {/* Assessments */}
      <section id="assessments" className="scroll-mt-20 border-b border-[#f3f4f6] py-20 sm:py-28">
        <div className="mx-auto grid max-w-7xl gap-12 px-5 sm:px-6 lg:grid-cols-[0.9fr_1.1fr] lg:items-start lg:px-8">
          <div className="lg:sticky lg:top-24">
            <p className="text-[13px] font-semibold tracking-[0.01em] text-[#4338ca]">{copy.home.assessmentsEyebrow}</p>
            <h2 className="mt-3 text-3xl font-semibold tracking-[-0.015em] text-[var(--it-text)] sm:text-4xl">{copy.home.assessmentsTitle}</h2>
            <p className="mt-5 text-base leading-7 text-slate-400">{copy.home.assessmentsBody}</p>
            <Link
              href={localePath("/signup", locale)}
              className="mt-8 inline-flex items-center gap-2 text-sm font-semibold text-[#4338ca] transition hover:text-[var(--it-text)]"
            >
              {copy.home.primaryCta}
              <ArrowRight className="h-4 w-4" strokeWidth={2} aria-hidden="true" />
            </Link>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {copy.home.assessmentGroups.map((group, i) => {
              ...
              return (
                <div
                  key={group.title}
                  className="rounded-2xl border border-[#f3f4f6] bg-[#ffffff] p-6 transition-colors duration-200 hover:border-[#d1d5db]"
                >
                  <div className="mb-4 flex h-9 w-9 items-center justify-center rounded-lg border border-[#4f46e5]/25 bg-[#4f46e5]/[0.08] text-[#4338ca]">
                    <Icon className="h-4 w-4" strokeWidth={1.8} aria-hidden="true" />
                  </div>
                  <p className="text-[15px] font-semibold text-[var(--it-text)]">{group.title}</p>
      ...
      <section id="faq" className="scroll-mt-20 border-b border-[#f3f4f6] bg-white py-20 sm:py-28">
        <div className="mx-auto max-w-3xl px-5 sm:px-6 lg:px-8">
          <div className="text-center">
            ...
            <h2 className="mt-3 text-3xl font-semibold tracking-[-0.015em] text-[var(--it-text)] sm:text-4xl">{copy.home.faqTitle}</h2>
          </div>
          <div className="mt-10 space-y-3">
            {copy.home.faq.map((item) => (
              <details key={item.question} className="group rounded-2xl border border-[#f3f4f6] bg-[#ffffff] px-6 py-5 open:border-[#d1d5db]">
                <summary className="flex cursor-pointer list-none items-center justify-between gap-4 text-[15px] font-semibold text-[var(--it-text)] focus:outline-none focus-visible:ring-2 focus-visible:ring-[#4338ca]/50 [&::-webkit-details-marker]:hidden">
                  {item.question}
                  <ChevronDown
      ...
      <section className="py-20 sm:py-28">
        <div className="mx-auto max-w-5xl px-5 sm:px-6 lg:px-8">
          <div className="gradient-ring relative overflow-hidden rounded-3xl px-6 py-16 text-center sm:px-16">
            <div className="absolute inset-0 bg-[radial-gradient(ellipse_60%_80%_at_50%_-10%,rgba(79,70,229,0.08),transparent_70%)]" aria-hidden="true" />
            <div className="relative">
              <h2 className="mx-auto max-w-2xl text-3xl font-semibold tracking-[-0.015em] text-[var(--it-text)] sm:text-4xl">
              ...
              <div className="mt-9 flex flex-col justify-center gap-3 sm:flex-row">
                ...
                <Link
                  ...
                  className="inline-flex items-center justify-center rounded-xl border border-[#f3f4f6] bg-[#f8fafc]/60 px-6 py-3.5 text-sm font-semibold text-[var(--it-text)] transition hover:border-[#d1d5db]"
                >
                  {copy.nav.contact}
                </Link>
              </div>
