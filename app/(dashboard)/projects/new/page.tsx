...
const ROLE_TEMPLATES: RoleTemplate[] = [
  {
    ...
    description: "Core reasoning, judgment, integrity, and decision-making battery, plus sales aptitude.",
    estimatedMinutes: 185,
    accentClass: "text-green-300",
    borderClass: "border-green-500/30 bg-green-500/8",
    dotClass: "bg-green-400",
    keywords: [...CORE_BATTERY_KEYWORDS, "sales aptitude"],
  ...
  },
  {
    ...
    description: "Core reasoning, judgment, integrity, and decision-making battery, plus customer service skills.",
    estimatedMinutes: 185,
    accentClass: "text-sky-300",
    borderClass: "border-sky-500/30 bg-sky-500/8",
    dotClass: "bg-sky-400",
    keywords: [...CORE_BATTERY_KEYWORDS, "customer service skills"],
  ...
  },
  {
    id: "manager",
    role: "Manager",
    description: "Core reasoning, judgment, integrity, and decision-making battery for management shortlists.",
    estimatedMinutes: 165,
    accentClass: "text-amber-300",
    borderClass: "border-amber-500/30 bg-amber-500/8",
    dotClass: "bg-amber-400",
    keywords: CORE_BATTERY_KEYWORDS,
  ...
  },
  {
    ...
    description: "Core reasoning, judgment, integrity, and decision-making battery, plus problem solving.",
    estimatedMinutes: 190,
    accentClass: "text-blue-300",
    borderClass: "border-blue-500/30 bg-blue-500/8",
    dotClass: "bg-blue-400",
    keywords: [...CORE_BATTERY_KEYWORDS, "problem solving"],
  ...
  },
  {
    id: "hr-manager",
    role: "HR Manager",
    description: "Core reasoning, judgment, integrity, and decision-making battery for HR shortlists.",
    estimatedMinutes: 165,
...
export default function NewProjectPage() {
  ...
  const copy = es
    ? {
        ...
        roles: {
          "sales-representative": { role: "Representante de ventas", description: "Batería central de razonamiento, juicio, integridad y toma de decisiones, más aptitud comercial." },
          "customer-service-agent": { role: "Agente de atención al cliente", description: "Batería central de razonamiento, juicio, integridad y toma de decisiones, más atención al cliente." },
          manager: { role: "Gerente", description: "Batería central de razonamiento, juicio, integridad y toma de decisiones para short-lists de gestión." },
          "software-developer": { role: "Desarrollador/a de software", description: "Batería central de razonamiento, juicio, integridad y toma de decisiones, más resolución de problemas." },
          "hr-manager": { role: "Gerente de RR. HH.", description: "Batería central de razonamiento, juicio, integridad y toma de decisiones para short-lists de RR. HH." },
          "call-center-agent": { role: "Agente de call center", description: "Batería central de razonamiento, juicio, integridad y toma de decisiones, más atención al cliente." },
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <nav className="flex items-center gap-2 text-sm">
        ...
        <span className="text-[var(--it-text)] font-medium">{copy.newProject}</span>
      </nav>

      <div>
        <h1 className="text-2xl font-bold text-[var(--it-text)]">{copy.createProject}</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-3 rounded-lg bg-white border border-[var(--it-border)] text-[var(--it-text)] focus:outline-none focus:border-[#4f46e5] focus:ring-1 focus:ring-[#4f46e5] transition-colors text-sm"
              />
            </div>
            <div className="sm:col-span-2">
              ...
              <textarea
                ...
                className="w-full px-4 py-3 rounded-lg bg-white border border-[var(--it-border)] text-[var(--it-text)] placeholder-[var(--it-faint)] focus:outline-none focus:border-[#4f46e5] focus:ring-1 focus:ring-[#4f46e5] transition-colors text-sm resize-none"
              />
            </div>
          </div>
        </div>
        ...
        {/* ── Role Template ────────────────────────────────────────── */}
        <div className="bg-[#ffffff] border border-[var(--it-hairline)] rounded-xl p-6">
          <div className="mb-5 border-b border-[var(--it-hairline)] pb-4">
            <h2 className="text-base font-semibold text-[var(--it-text)]">{copy.roleTemplate}</h2>
            ...
            {ROLE_TEMPLATES.map((template) => {
              ...
              return (
                <button
                  ...
                  className={`group relative flex flex-col items-start gap-3 rounded-xl border p-4 text-left transition-all focus:outline-none focus:ring-2 focus:ring-[#4f46e5]/50 ${
                    isSelected
                      ? "border-[#4f46e5] bg-[#4f46e5]/8 ring-1 ring-[#4f46e5]/30"
                      : isCustom
                      ? "border-dashed border-slate-600/50 bg-[#f8fafc]/40 hover:border-slate-500/70"
                      : "border-[var(--it-hairline)] bg-[var(--it-surface-muted)] hover:border-[var(--it-border)]"
                  }`}
                >
                  {/* Selected checkmark */}
                  {isSelected && (
                    <span className="absolute top-3 right-3 flex h-5 w-5 items-center justify-center rounded-full bg-[#4f46e5]">
                      <svg className="h-3 w-3 text-[var(--it-text)]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <span className={`flex h-9 w-9 items-center justify-center rounded-lg border ${
                    isSelected
                      ? "border-[#4f46e5]/40 bg-[#4f46e5]/15 text-blue-300"
                  ...
                  <div className="min-w-0 flex-1 pr-4">
                    <p className={`text-sm font-semibold ${isSelected ? "text-[var(--it-text)]" : "text-slate-200"}`}>
                      {templateCopy.role}
                    </p>
                  ...
                  {!isCustom && (
                    <div className="flex flex-wrap gap-2">
                      <span className="inline-flex items-center gap-1 rounded-full border border-[var(--it-hairline)] bg-[#f8fafc] px-2 py-0.5 text-[10px] font-medium text-slate-400">
          ...
          {activeTemplate && (
            <div className="mt-4 flex items-center gap-2 rounded-lg border border-[var(--it-hairline)] bg-[#f8fafc] px-3 py-2">
        ...
        <div className="bg-[#ffffff] border border-[var(--it-hairline)] rounded-xl p-6">
          <div className="mb-5 flex flex-col gap-3 border-b border-[var(--it-hairline)] pb-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h2 className="text-base font-semibold text-[var(--it-text)]">
            ...
            <div className="flex shrink-0 items-center gap-2">
              {selectedAssessments.length > 0 && (
                <span className="inline-flex items-center gap-1 rounded-full border border-[var(--it-hairline)] bg-[#f8fafc] px-3 py-1 text-xs font-medium text-slate-400">
              ...
              <span className="inline-flex w-fit items-center rounded-full border border-[var(--it-hairline)] bg-[#f8fafc] px-3 py-1 text-xs font-medium text-slate-400">
        ...
        <div className="flex items-center gap-3 justify-end">
          <Link
            href="/projects"
            className="px-5 py-2.5 rounded-lg border border-[var(--it-hairline)] text-slate-400 hover:text-slate-200 hover:border-[#d1d5db] text-sm font-medium transition-colors"
          >
            {copy.cancel}
          </Link>
          <button
            type="submit"
            disabled={saving}
            className="inline-flex items-center gap-2 px-5 py-2.5 rounded-lg bg-[#4f46e5] hover:bg-[#3730a3] text-white text-sm font-semibold transition-colors disabled:opacity-60 disabled:cursor-not-allowed"
          >
            {saving ? (
              <>
