...
exports.buildAssessmentIntelligence = buildAssessmentIntelligence;
const aq_1 = require("./extractors/aq");
const critical_thinking_1 = require("./extractors/critical-thinking");
const score_only_1 = require("./extractors/score-only");
const scales_1 = require("./scales");
const taxonomy_1 = require("./taxonomy");
const COPY = {
    es: {
        noRoleModel: "No se proporciono un modelo de competencias del rol; el informe evalua desempeno en evaluaciones completadas, no ajuste completo al puesto.",
        ...
        evidenceCount: (count) => `${count} evaluacion${count === 1 ? "" : "es"} completada${count === 1 ? "" : "s"} analizada${count === 1 ? "" : "s"}.`,
        summaryHeadline: "Recomendacion basada en evidencia completada y trazable.",
        summaryBody: "La recomendacion usa senales de evidencia extraidas de evaluaciones completadas. Las conclusiones se limitan a los instrumentos disponibles y deben validarse con entrevista estructurada.",
    ...
    en: {
        noRoleModel: "No role competency model was provided; this report evaluates completed assessment performance, not full role fit.",
        ...
        evidenceCount: (count) => `${count} completed assessment${count === 1 ? "" : "s"} analyzed.`,
        summaryHeadline: "Recommendation based on completed and traceable evidence.",
        summaryBody: "The recommendation uses evidence signals extracted from completed assessments. Conclusions are limited to the available instruments and should be validated through a structured interview.",
...
function average(values) {
    if (!values.length)
        return 0;
    return (0, scales_1.clampScore)(values.reduce((sum, value) => sum + value, 0) / values.length);
}
function copy(locale) {
    return COPY[locale];
}
function extractSignals(input, locale) {
    const key = (0, scales_1.assessmentKey)(input.name);
    if (key === "critical-thinking")
        return (0, critical_thinking_1.extractCriticalThinkingEvidence)(input, locale);
    if (key === "aq")
        return (0, aq_1.extractAQEvidence)(input, locale);
    return (0, score_only_1.extractScoreOnlyEvidence)(input, locale);
}
function dominantDirection(signals) {
    if (signals.some((signal) => signal.direction === "risk"))
        return "risk";
    if (signals.some((signal) => signal.direction === "mixed"))
        return "mixed";
    if (signals.some((signal) => signal.direction === "positive"))
        return "positive";
    return "neutral";
}
function buildCompetencyEvidence(signals, locale) {
    ...
    return Array.from(grouped.values()).map((items) => {
        ...
        return {
            ...
            category: (0, taxonomy_1.competencyCategory)(strongest.competencyId, locale),
            score,
            direction: dominantDirection(items),
            strength: (0, scales_1.evidenceStrength)(score),
            evidenceSignalIds: items.map((item) => item.id),
            summary: strongest.businessImpact,
        };
    });
}
function hasMixedEvidence(signals) {
...
function buildRisks(signals, locale) {
    return signals
...
function questionForStrength(signal, locale) {
    return {
...
function buildInterviewQuestions(signals, risks, locale) {
...
function calculateConfidence({ signals, assessmentCount, risks, mixed, roleRequirementsProvided, locale, }) {
    const c = copy(locale);
    let score = 35;
    const factors = [];
    const limitations = [];
    ...
    if (assessmentCount >= 2) {
        score += 15;
        factors.push(c.evidenceCount(assessmentCount));
    }
    ...
    if (mixed) {
        score -= 18;
        limitations.push(c.mixedEvidence);
    }
    if (hasScoreOnly) {
        score -= 8;
        limitations.push(c.unsupported);
    }
    if (!roleRequirementsProvided) {
        score -= 8;
        limitations.push(c.noRoleModel);
    }
    if (assessmentCount === 1) {
        score = Math.min(score, 49);
        limitations.push(c.singleAssessment);
    }
    if (!roleRequirementsProvided)
        score = Math.min(score, 74);
    if (mixed)
        score = Math.min(score, 59);
    const bounded = (0, scales_1.clampScore)(score);
    return {
        score: bounded,
        level: bounded >= 75 ? "high" : bounded >= 55 ? "moderate" : "low",
        factors: factors.length ? factors : [c.evidenceCount(assessmentCount)],
        limitations: Array.from(new Set(limitations)),
    };
}
function buildRecommendation({ assessmentAverage, confidence, risks, mixed, evidenceSignalIds, roleRequirementsProvided, locale, }) {
    const c = copy(locale);
    const hasHighRisk = risks.some((risk) => risk.severity === "high");
    const hasRisk = risks.length > 0;
    let level = "review";
    let title = c.reviewTitle;
    let rationale = c.reviewRationale;
    if (assessmentAverage >= 85 && !hasRisk && !mixed && confidence.level === "high") {
        level = "strong";
        title = c.strongTitle;
        rationale = c.strongRationale;
    }
    else if (assessmentAverage >= 70 && !hasHighRisk && !mixed) {
        level = "proceed";
        title = c.proceedTitle;
        rationale = c.proceedRationale;
    }
    else if (assessmentAverage < 55 || hasHighRisk) {
        level = "caution";
        title = c.cautionTitle;
        rationale = c.cautionRationale;
    }
    const limitations = confidence.limitations;
    if (!roleRequirementsProvided && !limitations.includes(c.noRoleModel)) {
        limitations.push(c.noRoleModel);
    }
    return {
        level,
        ...
        nextSteps: [c.defaultStep, c.roleStep],
        limitations,
    };
}
function buildAssessmentIntelligence(options) {
    ...
    const assessmentAverage = average(assessments.map((assessment) => (0, scales_1.clampScore)(assessment.score)));
    ...
    return {
        ...
        recommendation,
        executiveSummary: {
            headline: copy(locale).summaryHeadline,
            summary: copy(locale).summaryBody,
            evidence: [
                copy(locale).evidenceCount(assessments.length),
                ...signals.slice(0, 3).map((signal) => signal.statement),
                ...confidence.limitations.slice(0, 2),
            ],
        },
        methodologyLimitations: Array.from(new Set([...confidence.limitations, ...signals.map((signal) => signal.limitation).filter(Boolean)])),
    };
}
