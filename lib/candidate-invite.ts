...
export type CandidateInviteDeliveryMethod = "email" | "link";
...
export interface CandidateInviteInput {
  ...
  deliveryMethod: CandidateInviteDeliveryMethod;
  token?: string | null;
...
export function isValidCandidateEmail(value: unknown): value is string {
...
export async function createCandidateInvite(input: CandidateInviteInput): Promise<CandidateInviteResult> {
  const fullName = typeof input.fullName === "string" ? input.fullName.trim().slice(0, 500) : "";
  const normalizedEmail = typeof input.email === "string" ? input.email.trim().toLowerCase() : "";
  if (!input.projectId) {
    return { ok: false, status: 400, error: "Missing required fields" };
  }
  if (input.deliveryMethod === "email" && !isValidCandidateEmail(normalizedEmail)) {
  ...
  let candidate: { id: string; token: string | null } | null = null;
  if (input.token) {
    ...
    if (existingCandidateError) {
      console.error("[invite/candidate] idempotency lookup FAILED:", existingCandidateError.message);
      return { ok: false, status: 500, error: "Failed to create candidate" };
    }
  ...
  if (!candidate) {
    const limitCheck = await assertWithinLimit(admin, input.companyId, "candidate");
    if (!limitCheck.ok) {
      return {
        ok: false,
        status: 403,
        error: limitCheck.message?.[lang] ?? "Candidate limit reached",
  ...
  if (bundle.length === 0) {
    return { ok: false, status: 400, error: "No assessments linked to this project" };
  }

  const assessment =
    (input.assessmentType ? bundle.find((item) => item.name === input.assessmentType) : undefined) ?? bundle[0];
  ...
  if (!candidate) {
    ...
    const { data: createdCandidate, error } = await admin
      .from("candidates")
      .insert({
    ...
    if (error) {
      console.error("[invite/candidate] create FAILED:", error.message);
      return { ok: false, status: 500, error: "Failed to create candidate" };
    }
  ...
  if (input.deliveryMethod === "email") {
    ...
    const toAddress = normalizedEmail;
    ...
    try {
      ...
      console.error("[invite/email] send THREW:", message);
      return {
        ok: false,
        status: 502,
        error: `Candidate created but email failed: ${message}`,
    ...
    if (resendResult.error) {
      ...
      return {
        ok: false,
        status: 502,
        error: `Candidate created but email failed: ${resendResult.error.message}`,
        candidateId: candidate.id,
        testUrl,
      };
    }
    ...
    return { ok: true, candidateId: candidate.id, testUrl, emailSent: true };
  }

  return { ok: true, candidateId: candidate.id, testUrl, emailSent: false };
}
