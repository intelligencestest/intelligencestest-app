export type AQDimension = "C" | "O" | "R" | "E";
...
export const AQ_DURATION_SECONDS = 20 * 60;
...
export function scoreAQ(answers: (number | null)[]): AQScoreResult {
  let control = 0, ownership = 0, reach = 0, endurance = 0;

  AQ_QUESTIONS.forEach((q, i) => {
    const raw = answers[i] ?? 3;
    const value = q.reversed ? 6 - raw : raw;
    if (q.dimension === "C") control += value;
    else if (q.dimension === "O") ownership += value;
    else if (q.dimension === "R") reach += value;
    else if (q.dimension === "E") endurance += value;
  });
