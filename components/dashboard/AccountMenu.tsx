...
export function AccountMenu({ userEmail, userName }: AccountMenuProps) {
  ...
  const links = [
    { href: "/settings#account", label: copy.account },
  ...
  return (
    <div className="relative">
      <button
        ...
        className="group inline-flex cursor-pointer items-center gap-3 rounded-full border border-[var(--it-border)] bg-[var(--it-card)]/92 py-1.5 pl-1.5 pr-3 text-left shadow-[0_18px_45px_rgba(0,0,0,0.2)] transition-colors hover:border-[#2f3a62] focus:outline-none focus:ring-2 focus:ring-[#1D4ED8]/50"
      >
        ...
        <span className="hidden min-w-0 sm:block">
          <span className="block max-w-[160px] truncate text-sm font-semibold text-slate-100">{displayName}</span>
          <span className="block max-w-[160px] truncate text-[11px] text-[var(--it-faint)]">{userEmail}</span>
        </span>
