...
export default async function ProcessHealthStrip({ data }: { data: ProcessHealthData }) {
  ...
  return (
    <section className="grid grid-cols-2 gap-4 xl:grid-cols-4">
      {items.map((item) => (
        <Link key={item.key} href={item.href} className="premium-card premium-card-hover rounded-xl p-4">
          <p className="text-xs font-medium text-slate-500">{item.label}</p>
          <div className="mt-1.5 flex items-baseline gap-2">
            <span className="text-xl font-semibold tracking-tight text-slate-200">{item.value}</span>
